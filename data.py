...
class FewShotLearningDatasetParallel(Dataset):
    ...
    def load_dataset(self):
        ...
        if self.args.load_into_memory is True:
            ...
            joblib.dump(dataset_splits, "datasets/ddi2013/meta_data_dict.pkl")
            self.data_loaded_in_memory = True

        return dataset_splits

    def get_df2data(self, df):
        #todo
        return df
    ...
    def get_data_paths(self):
        ...
        :return: data_image_paths: dict containing class to filepath list pairs.
                 ...
                 label_to_index: dictionary containing human understandable string mapped to numerical indexes
        """
        print("Get smiles from", self.data_path)
    ...
    def load_batch(self, batch_image_paths):
        """
        Load a batch of smiles, given a list of filepaths
        :param batch_image_paths: A list of filepaths
        :return: A numpy array of smiles of shape batch, height, width, channels
        """
        image_batch = []
    ...
    def get_set(self, dataset_name, seed):
        ...
        for class_entry in selected_classes:
            ...
            x_smiles.append(class_image_samples)
            y_labels.append(class_labels)

        x_smiles = torch.stack(x_smiles)
        y_labels = np.array(y_labels, dtype=np.float32)

        support_set_smiles = x_smiles[:, :self.num_samples_per_class]
        support_set_labels = y_labels[:, :self.num_samples_per_class]
        target_set_smiles = x_smiles[:, self.num_samples_per_class:]
        target_set_labels = y_labels[:, self.num_samples_per_class:]

        return support_set_smiles, target_set_smiles, support_set_labels, target_set_labels, seed
    ...
    def __getitem__(self, idx):
        support_set_x, target_set_x, support_set_labels, target_set_label, seed = \
            self.get_set(self.current_set_name, seed=self.seed[self.current_set_name] + idx)

        return support_set_x, target_set_x, support_set_labels, target_set_label, seed
